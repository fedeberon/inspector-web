...
<div class="content">
    <div class="col-12">
        ...
        <div class="card">
            <form:form action="saveUpdate" modelAttribute="appUbicacionRelevamiento" method="post">
            <form:hidden path="id" value='${appUbicacionRelevamiento.id}'/>
            <form:hidden path="appUbicacion.id" value='${appUbicacionRelevamiento.appUbicacion.id}'/>
            <form:hidden path="appRelevamiento.id" value='${appUbicacionRelevamiento.appRelevamiento.id}'/>
            ...
            <div class="row ml-3">
                <div class="col-md-11">
                    <div class="form-group">
                        <div class="row">
                            <div class="col-6">
                                <label class="control-label pt-2">Agregar una ruta</label>
                                <select onchange="showRespuestaInputs()" id="select-relevamientos" class="form-control" name="appRelevamiento.id">
                                  <option value="-1"> Seleccionar nueva ruta</option>
                                    <c:forEach items="${relevamientos}" var="bo" varStatus="status">
                                        <option
                                        <c:forEach items="${updateRespuestas}" var="re" varStatus="status">
                                            <c:set var="contain" value="${re.appRelevamiento.id}" />
                                            <c:if test = "${fn:contains(contain, bo.id)}">
                                                disabled
                                            </c:if>
                                        </c:forEach>

                                        id="option-${bo.id}" value="${bo.id}"
                                        >
                                                ${bo.nombreRelevamiento}
                                        </option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div id="oneOrdenS-select" class="col-6 hidden">
                                <label for="oneOrdenS" class="control-label pt-2 pr-2"><strong style="color:red;">*</strong> ONE Orden_S</label>
                                <form:input type="number" path="oneOrdenS"   min="0" cssClass="form-control" id="oneOrdenS" placeholder="*Obligatorio"/>
                            </div>
                            <div id="oneCircuito-select" class="col-6 hidden">
                                <label for="oneCircuito" class="control-label pt-2 pr-2"> ONE Circuito</label>
                                <form:input type="number" path="oneCircuito" min="0" cssClass="form-control" id="oneCircuito" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="cantidad" class="control-label pt-2 pr-2 col-4">Cantidad</label>
                                <form:input path="cantidad" type="number" cssClass="form-control" id="cantidad" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="evp" class="control-label pt-2 pr-2 col-4">EVP</label>
                                <form:input path="evp" cssClass="form-control" id="evp" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="elemento" class="control-label pt-2 pr-2 col-4">Elemento</label>
                                <form:input path="elemento" cssClass="form-control" id="elemento" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="anunciante" class="control-label pt-2 pr-2 col-4">Anunciante</label>
                                <form:input path="anunciante" cssClass="form-control" id="anunciante" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="producto" class="control-label pt-2 pr-2 col-4">Producto</label>
                                <form:input path="producto" cssClass="form-control" id="producto" placeholder=""/>
                            </div>
                            <div class="col-6">
                                <label for="referencias" class="control-label pt-2 pr-2 col-4">Referencias</label>
